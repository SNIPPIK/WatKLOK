...
import {ActionType, Event} from "@handler";
import {ActionMessage, SuperClient} from "@Client";
...
export default class extends Event<Events.InteractionCreate> {
     ...
     * @private
     */
    private readonly _stepCommand = (message: ClientInteraction): ActionType.command | Promise<ActionType.command> => {
     ...
     * @private
     */
    private readonly _stepButton = (message: ClientInteraction): ActionType.command => {
        ...
        switch (message.customId) {
            ...
            //Кнопка очереди
            case "queue": return db.commands.get("queue").execute(message, ["1"]) as ActionType.command;

            //Кнопка пропуска
            case "skip": return db.commands.get("skip").execute(message, ["1"]) as ActionType.command;

            //Кнопка повтора
            case "repeat": return db.commands.get("repeat").execute(message, [queue.options.loop === "songs" ? "song": "songs"]) as ActionType.command;

            //Кнопка паузы
            case "resume_pause": {
                //Если плеер играет
                if (queue.player.status === "playing") return db.commands.get("pause").execute(message) as ActionType.command;

                //Если плеер стоит на паузе
                else if (queue.player.status === "pause") return db.commands.get("resume").execute(message) as ActionType.command;
     ...
     * @param message {ClientInteraction} Взаимодействие с ботом
     * @param data {ActionType.command} Тип данных для отправки
     * @readonly
     * @private
     */
    private readonly _sendChannel = (message: ClientInteraction, data: ActionType.command | Promise<ActionType.command>) => {
        if (!(data instanceof Promise)) return void new ActionMessage({...data, message});
        data.then(data => new ActionMessage({...data, message})).catch(Logger.error);
    };
