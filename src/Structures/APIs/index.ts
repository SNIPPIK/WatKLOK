...
class LoadingAPIs {
    ...
    private get platform(): platform {
        return this.path.split("/")[0].toUpperCase() as platform;
    };
    public constructor(path: string) { this.path = path; };
    //====================== ====================== ====================== ======================
    /**
     * @description Загружаем классы
     */
    public readDir = () => {
        const files = readdirSync(`./src/Structures/APIs/${this.path}`);
        const Platform = Platforms.all.find(data => data.name === this.platform);
        const index = Platforms.all.indexOf(Platform);

        for (let file of files)
            this.findExport(`./${this.path}/${file}`).then(pl => Platforms.all[index].requests.unshift(pl));
...
if (Platforms.audio.length === 0) (async () => {
    ...
    ["YouTube", "Yandex", "Discord", "VK", "Spotify"].forEach((name) => new LoadingAPIs(`${name}/Classes`).readDir());
})();
