...
class Interaction {
    ...
    public get runCommand(): void {
        ...
        if (command.isOwner) {
            ...
            if (!OwnerIDS.includes(author.id)) return void (this.ChannelSend = { text: `${author}, ты слишком быстро отправляем сообщения! Подожди ${Duration.parseDuration(CoolDownBase.get(author.id).time)}`, color: "DarkRed" });
        }

        //Проверяем находится ли пользователь в базе
        if (CoolDownBase.get(author.id) && !OwnerIDS.includes(author.id)) return void (this.ChannelSend ={ text: `${author}, ты слишком быстро отправляем сообщения! Подожди ${Duration.parseDuration(CoolDownBase.get(author.id).time)}`, color: "DarkRed" });
        else {
            //Добавляем пользователя в CoolDown базу
            CoolDownBase.set(author.id, { time: command.cooldown });
            setTimeout(() => CoolDownBase.delete(author.id), command.cooldown * 1e3 ?? 5e3);
        }
