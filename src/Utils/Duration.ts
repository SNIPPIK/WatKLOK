...
export namespace Duration {
    ...
    export function getTracksTime(songs: Song[] | ISong.track[]): string {
        let time: number | string = "00:00";

        //Если трек треков
        if (songs.length === 0) return time;

        //Если есть хотя бы один трек
        time = 0; for (const song of songs) time += parseInt(song.duration.seconds as string);

        return toConverting(time) as string;
    }
