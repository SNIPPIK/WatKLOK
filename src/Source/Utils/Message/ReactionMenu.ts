...
import {MessageUtils} from "@Utils/Message/index";
...
export class ReactionMenu {
    public constructor(embed: EmbedData | string, message: ClientMessage | ClientInteraction, callbacks: ReactionCallbacks) {
        const args: any = typeof embed === "string" ? { content: embed, fetchReply: true } : { embeds: [embed], fetchReply: true };

        if ("replied" in message && !(message as any).replied) message.reply(args).then((msg: any) => this.createReactions(msg, message, callbacks));
        else message.channel.send(args).then((msg: any) => this.createReactions(msg, message, callbacks));
    };
    //====================== ====================== ====================== ======================
    /**
     ...
     * @returns 
     */
    private readonly createReactions = (msg: ClientMessage, message: ClientMessage | ClientInteraction, callbacks: ReactionCallbacks) => Object.entries(callbacks).forEach(([key, value]) => {
