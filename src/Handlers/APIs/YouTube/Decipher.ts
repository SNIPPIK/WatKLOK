...
export function extractSignature(format: YouTubeFormat, html5player: string): Promise<YouTubeFormat> {
    return new Promise<YouTubeFormat>((resolve) => {
        ...
        try {
            ...
            new httpsClient(html5player, {proxy: true}).toString.then((page: string) => {
...
function extractFunctions(html5Link: string): Promise<string[]> {
    ...
    return new Promise((resolve, reject) => new httpsClient(html5Link, {proxy: true}).toString.then((body: string) => {
        if (!body) return;
