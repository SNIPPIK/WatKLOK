...
import {API, APIs} from "@handler/APIs";
    ...
    public get resource(): Promise<string | Error> {
        ...
        return new Promise(async (resolve) => {
            ...
            for (let req = 0; req < 3; req++) {
                try {
                    if (!this._link) this._link = await resource(platform, this.url, this.author.title, this.title, this.duration.seconds);

                    if (await new httpsClient(this.link, {method: "HEAD"}).status) break;
                    else this._link = null;
                } catch { this._link = null; break; }
            }

            //Если не удалось получить ссылку
