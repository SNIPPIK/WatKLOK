...
namespace FFmpegFilters {
     ...
     * @param filters {FFmpegFilters} Аудио фильтры которые включил пользователь
     */
...
function filtersForEach(filters: Filters, callback: (fl: string, filter: Filter) => void): void {
    filters.forEach((filter: string | number) => {
        ...
        return callback(filter, FFmpegFilters.get(filter));
    });
}
