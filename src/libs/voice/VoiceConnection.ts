...
import { Networking } from "./VoiceSocket";
...
export class VoiceConnection extends TypedEmitter<VoiceConnectionEvents & VoiceConnectionEventsSignals> {
     ...
     * @private
     */
    private onNetworkingStateChange(oldState: Networking.State, newState: Networking.State) {
        ...
        if (newState.code === Networking.StatusCode.Ready) this.state = { ...this.state, status: VoiceConnectionStatus.Ready };
        else if (newState.code !== Networking.StatusCode.Closed) this.state = { ...this.state, status: VoiceConnectionStatus.Connecting };
    };
