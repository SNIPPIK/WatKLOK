...
class Platform {
    ...
    /**
     * @description Загружаем данные для запросов
     */
    public static get init() {
        if (Platforms.audio.length === 0) {
            ...
            ["YouTube", "Yandex", "Discord", "VK", "Spotify"].forEach((platform) => {
                ...
                new initDataDir<API.list | API.array | API.track>(`Data/APIs/${platform}/Classes`, (file, data) => {
                    //Добавляем ошибки если они как таковые есть
                    if (!data) reason = "Not found exports";
                    else if (!data.callback) reason = "callback has not found";

                    if (reason) return Logger.error(`[APIs]: [${platform}.${data.type}]: ${reason}!`);
