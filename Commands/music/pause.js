...
class CommandPause extends Constructor_1.Command {
    constructor() {
        super({
            name: "pause",
        ...
        this.run = async (message) => {
            ...
            if (queue.player.state.status === 'paused')
                return message.client.Send({ text: `${message.author}, ⚠ | Музыка уже приостановлена!`, message: message, color: 'RED' });
